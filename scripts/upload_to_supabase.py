...
def load_csv_files():
    ...
    for table_name, file_path in data_files.items():
        if not os.path.exists(file_path):
            ...
            logger.info("Please run 'make data' or 'python scripts/generate_data.py' first")
            sys.exit(1)
...
def create_database_connection():
    ...
    if not database_url:
        ...
        logger.info("DATABASE_URL='postgresql://postgres.[PROJECT]:[PASSWORD]@aws-0-us-west-1.pooler.supabase.com:6543/postgres'")
    ...
    try:
        ...
        logger.info("Please check your DATABASE_URL and ensure Supabase project is running")
        sys.exit(1)
        ...
        try:
            with engine.connect() as conn:
                ...
                if db_count == csv_count:
                    ...
                    logger.warning(f"⚠️ {table_name}: {db_count} in DB vs {csv_count} in CSV")
...
def main():
    ...
    logger.info("You can now query your data in Supabase or connect to it from other applications")
