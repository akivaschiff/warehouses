...
def main():
    ...
    wheat = client.find(
        ...
        {"item_type": "Wheat"},
        limit=5,
        order_by="price_paid_usd",
        order_desc=True,
    )
    if len(wheat) > 0:
        print(
            wheat[
                ["item_type", "quantity", "unit", "price_paid_usd", "timestamp"]
    ...
    expensive = client.find(
        "exchanges",
        {"price_paid_usd__gte": 100000},
        limit=5,
        order_by="price_paid_usd",
        order_desc=True,
    )
    ...
    if len(recent) > 0:
        print(
            recent[
                ["timestamp", "item_type", "price_paid_usd", "brand_manufacturer"]
            ].to_string(index=False)
        )
    ...
    if len(volume_data) > 0:
        ...
        volume_data["total_value"] = volume_data["total_value"].apply(
            lambda x: f"${x:,.0f}"
        )
        volume_data["avg_value"] = volume_data["avg_value"].apply(
            lambda x: f"${x:,.0f}"
        )
    ...
    if len(monthly_data) > 0:
        monthly_data["total_value"] = monthly_data["total_value"].apply(
            lambda x: f"${x:,.0f}"
        )
    ...
    if len(steel_2023) > 0:
        print(
            steel_2023[["timestamp", "quantity", "unit", "price_paid_usd"]].to_string(
                index=False
            )
        )
    else:
