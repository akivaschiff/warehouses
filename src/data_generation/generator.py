            ...
            for _ in range(num_warehouses):
                warehouse = {
                    ...
                    "warehouse_type": random.choice(
                        ["distribution", "storage", "manufacturing", "retail"]
                    ),
                }
    ...
    def generate_commodity_exchange(self, date):
        ...
        if is_mint:
            ...
            to_warehouses = [
                ...
                if w["country"] == to_country
                and w["warehouse_type"] in ["manufacturing", "distribution"]
            ]
            to_warehouse = (
                random.choice(to_warehouses)["warehouse_id"]
                if to_warehouses
                else random.choice(self.warehouses)["warehouse_id"]
            )
            ...
            from_warehouses = [
                w for w in self.warehouses if w["country"] == from_country
            ]
            to_warehouses = [w for w in self.warehouses if w["country"] == to_country]

            from_warehouse = (
                random.choice(from_warehouses)["warehouse_id"]
                if from_warehouses
                else random.choice(self.warehouses)["warehouse_id"]
            )
            to_warehouse = (
        ...
        price_per_unit = (
            base_price
            * seasonal_multiplier
            * random.uniform(1 - volatility, 1 + volatility)
        )

        # Generate realistic quantity based on commodity type
        if commodity == "crude_oil":
            quantity = round(
                random.uniform(5000, 100000), 1
            )  # barrels (realistic shipment sizes)
            unit = "barrels"
        elif commodity == "copper":
            quantity = round(
                random.uniform(5, 500), 2
            )  # tons (smaller, more valuable shipments)
            unit = "tons"
            ...
            quantity = round(
                random.uniform(50, 2000), 1
            )  # tons (bulk commodity shipments)
            unit = "tons"
    ...
    def generate_serialized_exchange(self, date):
        ...
        if is_mint:
            ...
            to_warehouses = [
                w
                for w in self.warehouses
                if w["country"] == to_country and w["warehouse_type"] == "manufacturing"
            ]
            to_warehouse = (
                random.choice(to_warehouses)["warehouse_id"]
                if to_warehouses
...
if __name__ == "__main__":
    ...
    if len(wheat_exchanges) > 0:
        ...
        wheat_exchanges["price_per_unit"] = (
            wheat_exchanges["price_paid_usd"] / wheat_exchanges["quantity"]
        )
    ...
    country_exports = (
        trades_by_country.groupby("country")["price_paid_usd"]
        .sum()
