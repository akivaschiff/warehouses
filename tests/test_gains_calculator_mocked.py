...
class TestWarehouseGainsWithMocks:
    ...
    def mock_supabase_client(self, mock_warehouse_data, mock_exchange_data):
        ...
        mock_client.find.side_effect = lambda table, filters: {
            'warehouses': mock_warehouse_data if filters.get('warehouse_id') == 'WH_TEST_123' else pd.DataFrame(),
            'exchanges': self._filter_exchanges(mock_exchange_data, filters)
        }[table]
        
        return mock_client
    
    def _filter_exchanges(self, exchange_data: pd.DataFrame, filters: Dict[str, Any]) -> pd.DataFrame:
    ...
    def test_pure_logic_with_exchange_objects(self, expected_results):
        ...
        assert wheat_gains.total_outflow_value == expected_results['wheat_outflow_value']
        assert wheat_gains.total_gain_loss == expected_results['wheat_gain_loss']
        assert wheat_gains.number_of_transactions == expected_results['wheat_transactions']
    ...
    def test_complete_flow_with_mocked_database(self, mock_client_class, mock_supabase_client, expected_results):
    ...
    def test_multiple_commodities(self):
        ...
        wheat_gains = next(c for c in report.gains_by_commodity if c.commodity_type == 'Wheat')
        steel_gains = next(c for c in report.gains_by_commodity if c.commodity_type == 'Steel')
    ...
    def test_calculation_validation(self):
        ...
        commodity_total_inflow = sum(c.total_inflow_cost for c in report.gains_by_commodity)
        commodity_total_outflow = sum(c.total_outflow_value for c in report.gains_by_commodity)
